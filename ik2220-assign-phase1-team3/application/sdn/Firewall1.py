		...
		# firewall in port 2 (DmZ to PbZ)	
			...
			if ip is not None:
				...
				if icmp is not None:
					dstIp = ip.dstip
					if icmp.type == 0:			
						if dstIp == '100.0.0.20' or dstIp =='100.0.0.21' or dstIp == '100.0.0.22' or dstIp == '100.0.0.40' or dstIp =='100.0.0.41' or dstIp == '100.0.0.42':
							drop()
							print("f1 p2: A packet is transmitted to (%s) by icmp protocol:drop." %dstIp)
							return
						else:
							super(LearningFireWall1, self)._handle_PacketIn(event)
							return
